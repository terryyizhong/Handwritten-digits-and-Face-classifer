...
def enhancedFeatureExtractorDigit(datum):
  ...
  for x in xrange(DIGIT_DATUM_WIDTH):
    for y in xrange(DIGIT_DATUM_HEIGHT):
     if (x, y) not in visitedregion and datum.getPixel(x, y) < 2:
        continuous += 1
        stack = [(x, y)]
        while stack:
          v = stack.pop()
          ...
          for neighbor in getNeighbors(*v):
            if datum.getPixel(*neighbor) < 2 and neighbor not in visitedregion:
              stack.append(neighbor)

  features["continuous1"] = continuous % 2
  features["continuous2"] = (continuous >> 1) % 2
  features["continuous3"] = (continuous >> 2) % 2
  ####
  ##4.symmetry
  ####
  differences = []
  for x in range(DIGIT_DATUM_WIDTH):
    ...
    differences.append(abs(topPix - bottomPix))
...
def enhancedFeatureExtractorFace(datum):
  ...
  # bottomPixels = 0
  # for x in range(FACE_DATUM_WIDTH):
  #   for y in range(FACE_DATUM_WIDTH / 2, FACE_DATUM_HEIGHT):
  #     if datum.getPixel(x, y) > 0:
  #       bottomPixels += 1
  #
  # if (topPixels - bottomPixels) > 0:
  ...
  for y in range(FACE_DATUM_HEIGHT):
    ...
    for x in range(FACE_DATUM_WIDTH / 2):
      if datum.getPixel(x, y) > 0:
        leftPix += 1
    for x in range(FACE_DATUM_WIDTH / 2, FACE_DATUM_WIDTH):
      if datum.getPixel(x, y) > 0:
        rightPix += 1
...
def readCommand( argv ):
  ...
  parser.add_option('-k', '--smoothing', help=default("Smoothing parameter (ignored when using --autotune)"), type="float", default=0.001)
